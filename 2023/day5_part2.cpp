...
struct Seed
{
    long long int seedStart;
    long long int seedRange;
};

std::vector<Seed> GetSeeds(const std::vector<std::string> &data)
{
    std::vector<long long int> seeds = parseHeader(data[0]);
    std::vector<Seed> seedsExpanded;
    long long int seedStart, seedRange;
    Seed tmpSeed;
    for (size_t i = 0; i < seeds.size(); i++)
    {
        ...
        if (i % 2 != 0)
        {
            tmpSeed.seedStart = seedStart;
            tmpSeed.seedRange = seedRange;
            seedsExpanded.push_back(tmpSeed);
        }
    }

    return seedsExpanded;
}
...
long long int LowestLocationNumber(std::string filename)
{
    ...
    std::vector<Seed> seeds = GetSeeds(data);

    std::cout << "---- SEEDS ----" << std::endl;
    // for (size_t i = 0; i < seeds.size(); i++)
    // {
    //     long long int seed = seeds[i].seedStart;
    //     while (seed < seeds[i].seedStart + seeds[i].seedRange)
    //     {
    ...
    for (size_t i = 0; i < seeds.size(); i++)
    {
        ...
        std::cout << "pair finished" << std::endl;
    }
