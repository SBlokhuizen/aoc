...
int CountSteps(std::string filename)
{
    ...
    // Start search
    auto it = nodeMap.find("AAA");
    int cntr = 0;
    int number;
    while (!((sequence[number] == 'L' && node.left == "ZZZ") || (sequence[number] == 'R' && node.right == "ZZZ")))
    {
        number = cntr % sequence.size();
        // Get the Node object
        node = it->second;
        if (sequence[number] == 'L')
        {
            it = nodeMap.find(node.left);
        }
        else if (sequence[number] == 'R')
        {
            it = nodeMap.find(node.right);
        }
        else
        {
        ...
        std::cout << sequence[number] << " | left: " << node.left << " | right: " << node.right << std::endl;
        cntr++;
    }
