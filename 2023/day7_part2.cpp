...
int CountJokers(PokerHand pokerHand)
{
    int numJokers = 0;
    for (char ch : pokerHand.hand)
    {
        if (ch == 'J')
        {
            numJokers++;
...
{
    int numJokers = CountJokers(pokerHand);
    int handType;
    std::string remainder;
    std::cout << "----" << std::endl;
    std::cout << pokerHand.hand << std::endl;
...
int ProductOfGames(std::string filename)
{
    ...
    {
        std::cout << sortedHands[i].hand << std::endl;
